...
ext = ".xz"
test_compute_targets_file = f'./test_data/compute_targets{ext}'
test_compute_bbox_targets_file = f'./test_data/compute_bbox_targets{ext}'
test_nms_file = './test_data/nms.pkl'
test_apply_bbox_deltas_file = f'./test_data/apply_bbox_deltas{ext}'
test_generate_anchors_file = f'./test_data/generate_anchors{ext}'
test_base_anchors_file = f'./test_data/base_anchor_data{ext}'
...
class TestClass(unittest.TestCase):
    ...
    def _test_generate_anchors(self, RetinaNet, test_data_file, rtol=1e-5, atol=1e-8):
        dt = utils.load_variables(test_data_file,ext)
    ...
    def _test_generate_anchors_base(self, RetinaNet, test_data_file, rtol=1e-5, atol=1e-8):
        dt = utils.load_variables(test_data_file,ext)
        model = RetinaNet(fpn=True, p67=True)
        image = torch.zeros((1, 3, 731, 1333)).float()
        outputs = torch.from_numpy(np.array(dt['output']))
        outs = model(image)
        anchors = []
        for _, _, anchor in outs:
            B, A, H, W = anchor.shape
            anchor = anchor.reshape(B, A // 4, 4, H, W).squeeze(0).cpu().numpy()
            anchors.append(anchor[..., H//2, W//2])
        anchors = torch.from_numpy(np.array(anchors))
        
        
        for i in range(len(anchors)):
            iou = compute_bbox_iou(anchors[i], outputs[i])
            max_values, max_indices = torch.max(iou,axis=1)
            ref,_ = torch.sort(max_indices)
            max_values_expected = torch.ones(9)
            ref_expected = torch.arange(0, 9)
            self.assertTrue(
                max_values.shape == max_values_expected.shape,
                f"f'Anchor boxes alignment: not enough boxes, expected: {max_values_expected.shape} got: {max_values.shape} "
            )
            self.assertTrue(
                np.allclose(max_values, max_values_expected, rtol=rtol, atol=atol),
                f'Anchor boxes alignment incorrect: boxes not getting generated in correct position'
            )
            self.assertTrue(
                ref.shape == ref_expected.shape,
                f"f'Anchor boxes alignment: not enough boxes, expected: {ref_expected.shape} got: {ref.shape} "
            )
            self.assertTrue(
                np.allclose(ref, torch.arange(0, 9), rtol=rtol, atol=atol),
                f'Anchor boxes alignment incorrect: boxes not getting generated in correct position'
            )
    ...
    def test_generate_anchors(self):
        ...
        self._test_generate_anchors_base(RetinaNet,test_base_anchors_file)
    ...
    def _test_compute_targets(self, compute_targets, test_data_file, rtol=1e-5, atol=1e-8):
        ...
        for i in range(len(dt)):
            ...
            gt_clss, gt_bboxes = compute_targets(anchor, cls, bbox)
